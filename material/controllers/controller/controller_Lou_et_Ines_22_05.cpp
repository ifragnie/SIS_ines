...
int main(int argc, char **argv) {
  ...
  int         f_est_cols = init_csv(f_est, "time, est_x, est_y, est_theta, sigma_x, sigma_y,sigma_theta,"); // <-- don't forget the comma at the end of the string!!
...
*/
    ...
    ////LOG DES VALEURS ODOMETRY ////////////
    //log_csv(f_est,f_est_cols, time, state[0],state[1],state[2],cov[0][0],cov[1][1],cov[2][2]);
    // Log the time and light and IMU data in a csv file
    //log_csv(f_est, f_est_cols, time, light, imu[0], imu[1], imu[2]);
